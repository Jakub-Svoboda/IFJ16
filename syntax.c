...
int runSyntaxAnalysis (FILE *f, tListOfInstr * list) {
	...
	int result = syntaxCheck(CLASS_BLOCK,f,tokenPtr,lookAheadPtr,list);
...
//***************classBlock*******************//
		case CLASS_BLOCK:
			...
			if(tokenPtr->type != token_EOF){
					if ((result=syntaxCheck( CLASS, f, tokenPtr, lookAheadPtr, list))		!= 0) {goto EXIT;}
					if ((result=syntaxCheck( CLASS_BLOCK, f, tokenPtr, lookAheadPtr, list))		!= 0) {goto EXIT;}
			}else{
				fprintf(stderr, "\nSyntax OK\n");
			}
			return 0;
...
//******************class*******************//
		case CLASS:
			if ((result=syntaxCheck( ID, f, tokenPtr, lookAheadPtr, list))					!= 0) {fprintf(stderr,"\nID\n");goto EXIT;}
			if ((result=syntaxCheck( LEFT_CURLY_BRACKET, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\n{\n");goto EXIT;}
			if ((result=syntaxCheck( CLASS_BODY, f, tokenPtr, lookAheadPtr, list))			!= 0) {fprintf(stderr,"\nCB\n");goto EXIT;}
			if ((result=syntaxCheck( RIGHT_CURLY_BRACKET_CURRENT, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\n}\n");goto EXIT;}
		...
		case CLASS_BODY:
			...
			switch (tokenPtr ->type){
				case token_bracketRightCurly:
					...
					break;
				case token_static:
					if ((result=syntaxCheck( TYPE, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\ntype\n");goto EXIT;}
					if ((result=syntaxCheck( ID, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nid\n");goto EXIT;}
					getModifiedToken(f,tokenPtr);
					//printType(tokenPtr);
					if (tokenPtr -> type == token_assign){
						...
						if (tokenPtr -> type == token_bracketLeftRound){
							if ((result=syntaxCheck( FUNCTION_DECLARE, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\n(\n");goto EXIT;}
							break;
						}
					}
...
//******************FUNCTION_DECLARE*******************//
		case FUNCTION_DECLARE:
			...
			if(tokenPtr -> type == token_bracketRightRound){
				if ((result=syntaxCheck( FN_BODY_BEGIN, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nFBB\n");goto EXIT;}
			}else{
				if ((result=syntaxCheck( TYPE_CURRENT, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nTYPE\n");goto EXIT;}
				if ((result=syntaxCheck( ID, f, tokenPtr, lookAheadPtr, list))		!= 0) {fprintf(stderr,"\nID\n");goto EXIT;}
				getModifiedToken(f,tokenPtr);
				//printType(tokenPtr);
...
//******************FN_BODY_BEGIN*******************//
		case FN_BODY_BEGIN:
			if ((result=syntaxCheck( LEFT_CURLY_BRACKET, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nLCB\n");goto EXIT;}
			if ((result=syntaxCheck( FN_BODY, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nFB\n");goto EXIT;}
...
//******************FN_BODY*******************//
		case FN_BODY:
			...
			switch(tokenPtr -> type){
				...
				case token_identifier:								//id
					...
					if(tokenPtr -> type == token_bracketLeftRound){	// id(
						if ((result=syntaxCheck( FN_CALL, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nFN_CALL\n");goto EXIT;}
						if ((result=syntaxCheck( SEMICOLON, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\n;\n");goto EXIT;}
					...
					break;
				case token_int:
					if ((result=syntaxCheck( LOCAL_VAR_DEC, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nLVD\n");goto EXIT;}
					if ((result=syntaxCheck( FN_BODY, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nFNB\n");goto EXIT;}
					break;
				case token_String:
					if ((result=syntaxCheck( LOCAL_VAR_DEC, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nLVD\n");goto EXIT;}
				...
				case token_while:
					...
					addrOfLabel1 = listGetPointerLast(list);
					if ((result=syntaxCheck( LEFT_ROUND, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\n(\n");goto EXIT;}
					runPrecedenceAnalysis(f,tokenPtr,1);
					if(tokenPtr -> type != token_bracketRightRound){fprintf(stderr,"\n)\n");goto EXIT;}
					if ((result=syntaxCheck( COMMAND_BLOCK_BEGIN, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\n(\n");goto EXIT;}
					if ((result=syntaxCheck( FN_BODY, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nFNB\n");goto EXIT;}
					return result;
				case token_if:
					if ((result=syntaxCheck( LEFT_ROUND, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\n(\n");goto EXIT;}
					...
					if(tokenPtr -> type != token_bracketRightRound){fprintf(stderr,"\n)\n");goto EXIT;}
					if ((result=syntaxCheck( COMMAND_BLOCK_BEGIN, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nCBB\n");goto EXIT;}
					if ((result=syntaxCheck( ELSE, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nELSE\n");goto EXIT;}
					if ((result=syntaxCheck( COMMAND_BLOCK_BEGIN, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nCBB\n");goto EXIT;}
					if ((result=syntaxCheck( FN_BODY, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nFNB\n");goto EXIT;}
					return result;
				case token_return:
			...
			if ((result=syntaxCheck( FN_BODY_CURRENT, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nFBC\n");goto EXIT;}
		...
		case FN_CALL:
			...
			switch (tokenPtr ->type){
				...
				case token_doubleNumber:
					runPrecedenceAnalysis(f,tokenPtr,0);
					if ((result=syntaxCheck( FN_CALL_COMMA, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nFN_CALL_COMMA\n");goto EXIT;}					
					return result;
					break;
				default:
					return -1;
...
//******************COMMAND_BLOCK_BEGIN*******************//
		case COMMAND_BLOCK_BEGIN:
			...
			switch (tokenPtr -> type){
				...
				if ((result=syntaxCheck( COMMAND_BLOCK, f, tokenPtr, lookAheadPtr, list))	!= 0) {fprintf(stderr,"\nCB\n");goto EXIT;}
					return result;
					break;
				default:
					return -1;
